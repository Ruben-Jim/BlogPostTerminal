...
@click.option('--content',
              help='Content of the blog post (use editor if not provided)')
...
def create(title, content, tags, author):
    ...
    try:
        ...
        console.print(
            f"[green]✓ Blog post created successfully with ID: {post_id}[/green]"
        )
...
def edit(post_id):
    ...
    try:
        ...
        if not post:
            console.print(
                f"[red]Error: Post with ID '{post_id}' not found[/red]")
            return
        ...
        new_tags_input = Prompt.ask("Tags (comma-separated)",
        ...
        console.print(
            f"[green]✓ Post '{post_id}' updated successfully[/green]")
...
def list(limit, tag, author):
    ...
    try:
        ...
        for post_id, post in posts:
            ...
            table.add_row(
                post_id, truncate_text(post.title, 30), post.author,
                truncate_text(tags_str, 20), format_date(post.created_at),
...
def search(query):
    ...
    try:
        ...
        if not results:
            console.print(
                f"[yellow]No posts found matching '{query}'[/yellow]")
            return

        console.print(
            f"[blue]Found {len(results)} post(s) matching '{query}':[/blue]")
...
def delete(post_id):
    ...
    try:
        ...
        if Confirm.ask("Are you sure you want to delete this post?"):
            ...
            console.print(
                f"[green]✓ Post '{post_id}' deleted successfully[/green]")
        else:
...
@click.option('--output-dir',
              default='./blog_export',
              help='Directory to export posts to')
@click.option('--format',
...
def export(output_dir, format):
    ...
    try:
        ...
        console.print(
            f"[green]✓ Exported {len(exported_files)} post(s) to '{output_dir}'[/green]"
        )
...
[green]Average Post Length:[/green] {stats['avg_post_length']} characters
        ...
        console.print(
            Panel(panel_content, title="Blog Statistics", border_style="blue"))
...
def display_blog_gui():
    ...
    if not posts:
        console.print(
            Panel(
                "[yellow]No blog posts found. Create your first post with:[/yellow]\n[cyan]python main.py create[/cyan]",
                title="🔍 Empty Blog",
                border_style="yellow"))
        return
    ...
    with console.status("[bold green]Loading your blog posts...",
                        spinner="dots"):
        time.sleep(1)
    ...
    layout["body"].split_row(Layout(name="sidebar", minimum_size=30),
    ...
    while True:
        try:
            ...
            choice = Prompt.ask("[yellow]Enter your choice (1-7)[/yellow]",
            ...
            if choice == "1":
                ...
                if post:
                    console.clear()
                    display_full_post(post, post_id)
                    Prompt.ask("[dim]Press Enter to return to GUI...[/dim]",
                               default="")
                    console.clear()
                ...
                if content:
                    tags_input = Prompt.ask(
                        "[blue]Tags (comma-separated)[/blue]", default="")
                    author = Prompt.ask("[blue]Author name[/blue]",
                                        default="Anonymous")
                ...
                if results:
                    console.print(
                        f"[green]Found {len(results)} posts matching '{query}':[/green]\n"
                    )
                    for i, (post_id, post) in enumerate(results, 1):
                        console.print(
                            f"[cyan]{i}.[/cyan] {post.title} ([dim]{post_id}[/dim])"
                        )
                    ...
                    try:
                        ...
                        if post_choice.isdigit():
                            ...
                            if 0 <= idx < len(results):
                                ...
                                display_full_post(post, post_id)
                                Prompt.ask(
                                    "[dim]Press Enter to return to GUI...[/dim]",
                                    default="")
                ...
                if posts:
                    ...
                    for post_id, post in posts:
                        tags_str = ", ".join(
                            post.tags) if post.tags else "None"
                        table.add_row(post_id, truncate_text(post.title, 30),
                                      post.author, truncate_text(tags_str, 20),
...
def create_sidebar_content(posts):
    ...
    for post_id, post in posts[:10]:  # Show first 10 posts
        ...
        post_branch.add(
            f"[dim]Created:[/dim] [magenta]{format_date(post.created_at)}[/magenta]"
        )
...
def create_main_content(posts):
    ...
    if not posts:
        return Panel("[yellow]No posts to display[/yellow]",
                     title="📝 Recent Posts",
                     border_style="yellow")
...
[dim]{truncate_text(post.content, 200)}[/dim]
    ...
    if len(posts) > 3:
        content_panels.append(
            Panel(f"[dim]... and {len(posts) - 3} more posts available[/dim]",
                  border_style="blue"))
